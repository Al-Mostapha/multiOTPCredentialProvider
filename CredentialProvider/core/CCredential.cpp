...
CCredential::CCredential(std::shared_ptr<MultiOTPConfiguration> c) : _config(c), _util(_config), _privacyIDEA(c->piconfig)
{
	_cRef = 1;
		...
		__out CREDENTIAL_PROVIDER_STATUS_ICON *pcpsiOptionalStatusIcon)
{
	...
	*/
	...
	else
	{
		...
		if (_piStatus != PI_AUTH_SUCCESS && _config->pushAuthenticationSuccessful == false)
		{
			if (_config->isSecondStep == false && _config->twoStepHideOTP)
			{
				...
				DebugPrint("Prepare for second step");
				_util.SetScenario(_config->provider.pCredProvCredential,
													_config->provider.pCredProvCredentialEvents,
													SCENARIO::LOGON_BASE);
			...
			else
			{
				...
				{
					errorMessage = isGerman ? L"Server nicht erreichbar oder falsches offline OTP!" : L"Server unreachable or wrong offline OTP!";
				}
	...
	if (pcpgsr)
	{
		if (*pcpgsr == CPGSR_NO_CREDENTIAL_FINISHED)
		{
			DebugPrint("CPGSR_NO_CREDENTIAL_FINISHED");
		}
		if (*pcpgsr == CPGSR_NO_CREDENTIAL_NOT_FINISHED)
		{
			DebugPrint("CPGSR_NO_CREDENTIAL_NOT_FINISHED");
		}
		if (*pcpgsr == CPGSR_RETURN_CREDENTIAL_FINISHED)
		{
			DebugPrint("CPGSR_RETURN_CREDENTIAL_FINISHED");
		}
		if (*pcpgsr == CPGSR_RETURN_NO_CREDENTIAL_FINISHED)
		{
			DebugPrint("CPGSR_RETURN_NO_CREDENTIAL_FINISHED");
		}
	}
	else
	{
		DebugPrint("pcpgsr is a nullpointer!");
	}
...
void CCredential::ShowErrorMessage(const std::wstring &message, const HRESULT &code)
{
	*_config->provider.status_icon = CPSI_ERROR;
	...
	if (code != 0)
		errorMessage += L" (" + to_wstring(code) + L")";
...
HRESULT CCredential::Connect(__in IQueryContinueWithStatus *pqcws)
{
	...
	if (isRemote)
	{
		...
		if (wcscmp(tempStr, L"2e") == 0 || wcscmp(tempStr, L"2d") == 0)
		{ // Compar? des chaines ou il y a un pointeur
	...
	if (_config->twoStepHideOTP && !_config->isSecondStep)
	{
		...
		if (_piStatus == PI_AUTH_SUCCESS)
		{
			// OTP valid - set flag to use default CP and skip multiOTP
			_config->bypassPrivacyIDEA = true;
			_config->doAutoLogon = true;
			DebugPrint("OTP valid 1 ....................");
			return S_OK;
		}
		else
		{
			// OTP invalid - show error and reset
